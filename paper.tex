...
\end{equation}
We note that taking the derivative with respect to $\alpha$ in Eq.~\eqref{eq:intmultgammadef} we have
\begin{equation}
\int_{\mathbf{S}_+^p}e^{-\textrm{tr}\left[SA\right]}|S|^{\alpha}\log |S|\,dS=\Gamma_{p}\left(\alpha+\frac{1+p}{2}\right)|A|^{\alpha+\frac{1+p}{2}}\left\{\log |A|+\gamma_{p}\left(\alpha+\frac{1+p}{2}\right)\right\}\label{eq:intderivative}
\end{equation}
and the integral \eqref{eq:firstlogpart} can be computed by utilizing this result directly. To compute the second nontrivial log term \eqref{eq:2ndlogpart}
\begin{equation}
