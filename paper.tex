...
When $\Omega$ and $\Sigma$ are both unknown, the updating steps given in Eqs.~\eqref{eq:Omega} and \eqref{eq:Sigma} make the likelihood of matrix--$t$ distribution increases monotonically but the parameters $\Omega_k$ and $\Sigma_k$ may not converge as $k$ increases due to the non-identifiability, matrix distributions are defined up to a multiplication constant. This issue has also been mentioned in \citep{2013arXiv1309.6609G} for the estimation of matrix normal distributions. However, instead of fixing a single component in one of the scale matrices, we suggest setting the mean trace ratio
\begin{equation}
\frac{\textrm{tr}[\Omega_k]/m}{\textrm{tr}[\Sigma_k]/p}
\end{equation}
of $\Omega_k$ and $\Sigma_k$ to 1 with proper scaling at the end of each M-step. This ensures the convergence of the algorithm and also avoids possible numerical issues due to the large scale separation between $\Omega_k$ and $\Sigma_k$ in the iterations.
